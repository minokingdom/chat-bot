...
const ProcessModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      ...
      {selectedImage && (
        <div
          className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-md flex flex-col animate-fade-in overflow-hidden"
        ...
        >
          ...
          <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-50 bg-gradient-to-b from-black/50 to-transparent" onClick={e => e.stopPropagation()}>
            <div className="text-white/80 text-sm font-medium px-2 py-1 bg-black/30 rounded-lg backdrop-blur text-shadow">
              {isZoomed ? "스크롤하여 이동하세요" : "화면을 클릭하면 확대됩니다"}
            </div>
            <button
              className="text-white/80 hover:text-white p-2 transition-colors bg-white/10 rounded-full hover:bg-white/20 backdrop-blur"
              onClick={() => setSelectedImage(null)}
            >
              <X size={24} />
            </button>
          ...
          {/* Scroll Container */}
          <div
            className={`flex-1 w-full h-full overflow-auto flex ${isZoomed ? 'items-start justify-start' : 'items-center justify-center'} custom-scrollbar p-4`}
            onClick={() => isZoomed ? setIsZoomed(false) : setIsZoomed(true)}
          >
            <img
              src={selectedImage}
              alt="Zoomed manual"
